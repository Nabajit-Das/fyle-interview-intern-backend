...
def test_get_assignments_in_graded_state_for_each_student():
    ...
    expected_result = Assignment.get_graded_assignments_count_for_each_student(Assignment)
...
def test_get_grade_A_assignments_for_teacher_with_max_grading():
    ...
    sql_result = db.session.execute(text(sql).params(teacher_id=1)).fetchall()
    ...
    sql_result = db.session.execute(text(sql).params(teacher_id=2)).fetchall()
